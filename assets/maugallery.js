(function($) {
  ...
  // Capture des touches fléchées du clavier pour naviguer dans la lightbox
  $(document).on('keydown', function(e) {
    const lightbox = $(".modal.show"); // Vérifie si une lightbox est ouverte
    if (lightbox.length > 0) {
      if (e.key === "ArrowLeft") {
        $.fn.mauGallery.methods.prevImage(lightbox.attr("id"));
      } else if (e.key === "ArrowRight") {
        $.fn.mauGallery.methods.nextImage(lightbox.attr("id"));
      }
    }
  });
